...
let filteredData = data.filter(item => item.trending === true)
const prices = [
  {
...
const { FilterEqual,
  ...
  // on Click functions give conditional logic so filters can be toggled on/off
  ...
  { filteredData.length === 0 ? <div className='no-results'><h1>No Results Found</h1></div> :      
      ...
      <Link to={`/products/${item.id}`} key={item.id}>
        <div className="grid-item" >
          <img className='item-img' src={item.category === "tshirt" ? ShirtNoBg : item.category === "hat" ? HatNoBg : HoodieNoBg} alt="" />
