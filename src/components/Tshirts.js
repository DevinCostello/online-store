...
const Tshirts = () => {
  ...
  const { data, error, pending } = useFetch('http://localhost:8000/products');
  let filteredData = [...data.filter(item => item.category === "tshirt")]
      ...
      filteredData.map((item) =>
        <Link to={`/products/${item.id}`} key={item.id}>
          <div className="grid-item" >
            <img className='item-img' src={item.category === "tshirt" ? ShirtNoBg : HatNoBg} alt="" />

            <Link to={`/quick/${item.id}`}>
              <button className="quick" 
              onClick={() => Navigate(`/quick/${item.id}`)}
              >Quick View</button>
              </Link>
